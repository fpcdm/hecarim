...
- (void)applyDecl:(ICssDecl *)decl baseUrl:(NSString *)baseUrl{
	...
	if([k isEqualToString:@"display"]){
		...
		//NSLog(@"floatType = %d", self.floatType);
		...
		//NSLog(@"clearType = %d", self.clearType);
		...
		//NSLog(@"w = %f, ratioW = %f", self.w, self.ratioWidth);
		...
		//NSLog(@"h = %f, ratioH = %f", self.h, self.ratioH);
	}else if([k isEqualToString:@"margin"]){
		needsLayout = YES;
		_margin = [self parseEdge:v];
		//NSLog(@"margin: %f %f %f %f", _margin.top, _margin.right, _margin.bottom, _margin.left);
		...
		//NSLog(@"padding: %f %f %f %f", _padding.top, _padding.right, _padding.bottom, _padding.left);
		...
		//NSLog(@"border-radius: %f", _borderRadius);
...
- (void)parseBackground:(NSString *)v baseUrl:(NSString *)baseUrl{
	...
	if(src){
		if([IKitUtil isDataURI:src]){
			...
			//NSLog(@"%@ load background image: %@", _view.name, src);
