...
- (UIImage *)loadImage:(NSString *)path callback:(void (^)(UIImage *))callback{
	...
	if([IKitUtil isHttpUrl:path]){
		...
		if(img){
			NSLog(@"load img from cache: %@", path);
			if(callback){
				callback(img);
			}
			return img;
		...
		{
			NSLog(@"load img from remote: %@", path);
		...
		NSLog(@"load img from local: %@", path);
		if(img){
			if(callback){
				callback(img);
			}
...
- (IStyleSheet *)loadCss:(NSString *)path{
	...
	if(_enableCssCache){
		...
		if(sheet){
			NSLog(@"load css from cache: %@", path);
			return sheet;
		}
	}
	...
	if([IKitUtil isHttpUrl:path]){
		...
		if(data){
			...
			if(text){
				NSLog(@"load css from remote: %@", path);
		...
		NSLog(@"load css from local: %@", path);
		if(!err){
			sheet = [[IStyleSheet alloc] init];
			[sheet parseCss:text baseUrl:baseUrl];
		}
